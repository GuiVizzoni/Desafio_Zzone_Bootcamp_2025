...
import { trackSearch, trackFilterApplied } from '@/lib/n8n';
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    if (searchQuery) {
      const timer = setTimeout(() => {
        trackSearch(searchQuery, filteredServices.length);
  ...
  useEffect(() => {
    if (selectedCategory !== 'all' || sortBy !== 'relevance' || priceRange[0] !== 0 || priceRange[1] !== 2000 || maxDeliveryDays !== 30) {
      trackFilterApplied({
  ...
  }, [selectedCategory, sortBy, priceRange, maxDeliveryDays]);
